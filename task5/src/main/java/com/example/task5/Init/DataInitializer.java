...
public class DataInitializer {
    ...
    public void init() {
        // Создаём посетителей
        Visitor v1 = new Visitor(1L, "Анна", 30, "Ж");
        Visitor v2 = new Visitor(2L, "Иван", 25, "М");
        visitorService.save(v1);
        visitorService.save(v2);

        // Создаём рестораны
        Restaurant r1 = new Restaurant(1L, "Bella Italia", "Итальянская кухня", CuisineType.ITALIAN, new BigDecimal("700"), BigDecimal.ZERO);
        Restaurant r2 = new Restaurant(2L, "Golden Dragon", "Азиатская кухня", CuisineType.ASIAN, new BigDecimal("500"), BigDecimal.ZERO);
        restaurantService.save(r1);
        restaurantService.save(r2);

        // Добавляем оценки
        Review review1 = new Review(1L, v1, r1, 5, "Отлично!");
        Review review2 = new Review(2L, v2, r1, 4, "Очень вкусно");
        Review review3 = new Review(3L, v2, r2, 3, "Нормально");
